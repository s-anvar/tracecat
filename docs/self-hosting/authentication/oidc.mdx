...
<Steps>
  ...
  <Step title="Update settings in Tracecat">
    You can also configure these values from the Organization Settings page or via the `/settings/oidc` API.
    The API exposes a read-only `oidc_client_secret_set` field to indicate when a secret has been stored.
  </Step>
